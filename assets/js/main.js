...
HYPER = {
  ...
  ready : function() {
    ...
    if ($(".slideshow").length){
      this.slideshow.init({timeout : 0, detailPager : true});
    }
    if ($(".accordion").length){
      this.accordion();
    }
  ...
  resize : function() {
    ...
    $('.splash-image').css('height', $(window).height() * .8);
  },
  fajax : {
    load : function(){
    ...
    detailPager : function(){
      this.slidesSelector.on('cycle-pre-initialize', function( event, opts ) {
        ...
        opts.slides.each(function (i,v){
          HTML = $(v.outerHTML);
          img = HTML.find('img');
          category = HTML.find('span.category');
          title = HTML.find('h3');
          
          if (i != 0){
            $div.append('<li>'+img[0].outerHTML+'</li>');
          }else{
            $div.append('<li class="active">'+img[0].outerHTML+'</li>');
          }
        });

        $('.detailPager li').on('click',function(){
          // $(this).addClass('active').siblings().removeClass('active');
          $('.slidePager span').eq($(this).index()).trigger('click');
        });
      });

      this.slidesSelector.on('cycle-after', function( event, opts ) {
        activeSpanIdx = $('.slidePager span.active').index();
        $('.detailPager li').eq(activeSpanIdx).addClass('active').siblings().removeClass('active');
      });
  ...
  mailchimpSignup : function() {
    $('#newsletterForm').submit(function(e){
      ...
      $.ajax({
        url : templateUrl + '/assets/scripts/add_to_mc.php', //path to add_to_mc file
        data: form.serialize(),
        begin : function(){ //When submitted, do this... 
          submitButton.addClass('loading');
          submitButton.val('Subscribing...');
        },
        success : function(response){ //When finished, do this...
